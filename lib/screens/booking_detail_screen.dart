...
class _BookingDetailScreenState extends State<BookingDetailScreen> {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            _buildInfoRow('Date', _booking!.bookingDate ?? 'N/A'),
            _buildInfoRow('Departure Time', _booking!.departureTime ?? 'N/A'),
  ...
  Widget _buildItemsCard() {
    // Items can be a JSON string or a List
    List<dynamic> itemsList = [];
    if (_booking!.items is List) {
      itemsList = _booking!.items as List;
    } else if (_booking!.items is String) {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            ...itemsList.map((item) {
              if (item is Map) {
                final name = item['item_name'] ?? item['name'] ?? 'Item';
                final qty = item['quantity'] ?? 1;
                final rate = item['rate'] ?? item['price'] ?? 0;
                return Padding(
